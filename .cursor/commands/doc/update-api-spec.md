...
このスラッシュコマンドは、`apps/backend` で実装済みの API をもとに `docs/api-spec.md` を最新化し、未実装のエンドポイントにマークを付与 / 解除します。

### ゴール

- **バックエンド（Gin）のルーティング定義**と**API仕様書（`docs/api-spec.md`）**を同期する。
...
- スクリプトを作成するのではなく、`docs/api-spec.md`を修正する。
...
2. **仕様書 `docs/api-spec.md` を解析し、各エンドポイントを特定する**
   - `docs/api-spec.md` を読み込む。
...
4. **（追加）実装を正として、パラメータ/レスポンス例を最新化する**
   ...
   - 差分がある場合は **実装を最新**として、`docs/api-spec.md` を修正する。
...
5. **（追加）仕様書に存在しない実装済みAPIを追記する**
   ...
   - 見つかった場合は、`docs/api-spec.md` に **新規エンドポイントセクション**（見出し行＋概要＋パラメータ＋レスポンス＋例）として追記する。
   - 追記先は関連セクション（例: Tags / Movies / Auth など）に合わせ、番号（`#### 5.1` のような連番）も既存規則に合わせて整える。

6. **ファイルの更新**
   - 上記ロジックに従って更新した内容で `docs/api-spec.md` を上書きする。
