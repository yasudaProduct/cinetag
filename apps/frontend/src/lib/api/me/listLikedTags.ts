...
/**
 * ログインユーザーがいいねしたタグ一覧を取得する
 * @param token 認証トークン（必須）
 * @param page ページ番号（デフォルト: 1）
 * @param pageSize 1ページあたり件数（デフォルト: 20）
 */
export async function listLikedTags(
  token: string,
  page = 1,
  pageSize = 20,
  ...
  const res = await fetch(`${base}/api/v1/me/liked-tags?${params}`, {
    method: "GET",
    headers: {
  ...
  if (!parsed.success) {
    throw new Error("いいねしたタグ一覧のレスポンス形式が不正です");
  }
